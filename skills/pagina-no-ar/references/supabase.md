...
Leia no bloco 7 da aula, quando a página já está no ar e bonita e falta o dado entrar. Leia também
quando alguém disser "o formulário diz obrigado mas não tem nada no banco".
...
O Postgres confere o GRANT **primeiro** e nega antes de olhar a policy. Ou seja: dá pra ter RLS certa,
policy certa, e o insert falhar. Os dois cadeados precisam estar na medida: `insert` liberado,
`select` fechado.

## Passo 1: o projeto

Pré-work, não é aula. Projeto **provisionado antes de segunda**, região **São Paulo**, porque
provisionar demora e demorar ao vivo é caro. Se o painel disser que você bateu o limite de projetos,
a saída é **criar uma organização nova** e o projeto dentro dela.

[CONFIRMAR no ensaio de 13/08: o limite de projetos ativos no free parece ser por organização e não
por conta; e projeto free pausa por inatividade depois de alguns dias. Projeto pausado é página que
...
**tudo de uma vez**: não roda pedaço por pedaço, não deixa o Claude "adaptar", não traduz nome de
coluna pra português. O `fetch` do template procura exatamente estes nomes.

```sql
create table public.leads (
  -- `identity` e NÃO `serial`, de propósito: com identity, o `grant insert` na
  -- tabela já cobre a geração do id. Com `serial` faltaria um `grant usage` na
  -- sequence, e o erro seria "permission denied", igual ao do grant faltando.
  ...
  -- `required` do HTML, antes de qualquer requisição. Sem `default false` de
  -- propósito, porque default mais check se contradizem e devolvem 400 com
  -- nome de constraint do Postgres na cara do inscrito.
  consent_lgpd  boolean not null check (consent_lgpd is true),
  -- o texto do consentimento vai congelado na linha: consentimento é a um texto
  -- específico, e se a redação mudar dia 19 as linhas antigas precisam continuar
  -- sabendo com o que a pessoa concordou. 2000 porque essa frase não está sob
  -- controle do inscrito e pode crescer numa rodada de visual.
...
-- revoke não tira nada (não havia nada) e está certo assim; em projeto antigo
-- ele é o que fecha o SELECT que vinha aberto de fábrica.
revoke all on table public.leads from anon, authenticated;
grant insert on table public.leads to anon;

-- pode inserir, e só. Não existe policy de select, então ninguém lê pela API
-- pública, nem com a chave que está no HTML.
...
não devia: `create table` não devolve linha nenhuma mesmo. Texto vermelho com `ERROR` aí sim é erro, e
a frase de conserto é *cola esta mensagem inteira no chat e me diz qual linha do bloco falhou*. Aviso
amarelo no `revoke` de projeto novo é `NOTICE`: revogar o que não existe não quebra nada.

## Passo 3: a URL, a chave, e por que ela ser pública está certo

Painel do projeto, engrenagem de **Project Settings**, aba **API**.
[CONFIRMAR: em 2026 o painel pode ter renomeado essa aba pra "API Keys".]
...
página funciona idêntica e o banco fica aberto pro mundo ler **e apagar** a sua lista de inscritos. Não
há sintoma. É vazamento de nome, e-mail e telefone de pessoa física com anúncio rodando. Se a errada já
foi publicada, **rotaciona em Project Settings antes de republicar**: trocar só o arquivo não resolve.

A publicável, essa fica visível no código-fonte e é assim que tem que ser. Ela não é o que segura o
banco, quem segura é a RLS mais o grant. Com ela na mão, um estranho consegue exatamente uma coisa:
inserir uma linha em `leads`. Não lê, não apaga, não enxerga outra tabela.

E o aviso que anda junto, porque circula muito conselho errado: **variável de ambiente não esconde nada
em página estática.** Env var protege segredo de servidor. Numa página que roda no navegador o valor é
embutido no arquivo e continua público. Não existe "esconder a chave", existe **usar a chave certa**.

## Passo 4: o `fetch` literal

Já vem pronto no `index.html`, no bloco marcado INTOCÁVEL. Você só preenche `SUPABASE_URL` e
`SUPABASE_KEY` no topo. Está aqui pra conferir que não foi reescrito numa rodada de visual:
...
E o `if (!r.ok)`: `fetch` resolve normalmente em 401, 400 e 415, então `try/catch` sozinho pega falha de
rede e **não** pega status HTTP. Sem essa checagem a página diz "inscrição confirmada" com a internet
desligada.
...
Esperado: **`201` sem corpo**. `401` é o passo 2 (grant ou policy). `400` falando de coluna é nome
divergente ou o cache de schema de 30 segundos.
...
e quem decide é **o corpo da resposta**, não só o número:
...
[CONFIRMAR: o plano trata o negado como `401`, e o item 8 do ensaio de 13/08 (conta nova de Supabase)
fecha se conta nova devolve mesmo 401. O `42501` no corpo é o discriminador confiável de qualquer jeito.]

**A linha de teste que o `curl` inseriu fica no banco.** Apagar só dá pelo **Table Editor** do painel,
porque o `anon` não tem `delete`, e isso é os dois cadeados jogando a seu favor uma segunda vez. Antes
de contar inscrito pro desafio, entra no Table Editor e apaga as linhas de teste.
...
- **Sem `UNIQUE` no e-mail nesta aula:** quem testa duas vezes com o próprio e-mail leva `409`, e ao
  vivo isso parece "quebrou". Dedup na leitura. O teto de dano já são os `CHECK` de tamanho.
- **Não existe rate limit publicado pro Data API no free** (a doc de rate limit é só de Auth). Trate
  como se não houvesse freio: honeypot no formulário e os `CHECK` são a mitigação disponível.
- **CAPTCHA nativo do Supabase é só de Auth**, não vale pro formulário.
- **MCP do Supabase** fica fora: é OAuth mais token pessoal, dez vezes ao vivo, e daria liberdade ao
  modelo justamente no ponto onde liberdade custa 401. Upgrade depois do desafio.
...
oficial. Isto é ponto de partida: valide a redação com um advogado ou com o Encarregado de Dados do seu
negócio antes de escalar tráfego.
